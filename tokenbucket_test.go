...
func (rateLimitSuite) TestTakeMaxDuration(c *gc.C) {
	for i, test := range takeTests {
		tb := newBucket(test.fillInterval, test.capacity, BucketWithClock(nil))
		for j, req := range test.reqs {
			if req.expectWait > 0 {
...
func (rateLimitSuite) TestRate(c *gc.C) {
	...
	tb = newBucket(100*time.Millisecond, 1, BucketWithQuantum(5))
...
func checkRate(c *gc.C, rate float64) {
	tb := newBucket(1, 1<<62, BucketWithRate(rate))
...
func BenchmarkNewBucket(b *testing.B) {
	for i := b.N - 1; i >= 0; i-- {
		newBucket(1, 1<<62, BucketWithRate(4e18))
	}
}
