...
type testRunner interface {
	...
	createLimiter(int, ...leakOption) Limiter
...
type runnerImpl struct {
	...
	clock       *clock.Mock
	constructor func(int, ...leakOption) Limiter
	count       atomic.Int32
...
func runTest(t *testing.T, fn func(testRunner)) {
	impls := []struct {
		...
		{
			name: "atomic",
			constructor: func(rate int, opts ...leakOption) Limiter {
...
// createLimiter builds a limiter with given options.
func (r *runnerImpl) createLimiter(rate int, opts ...leakOption) Limiter {
...
func TestPer(t *testing.T) {
	t.Parallel()
	runTest(t, func(r testRunner) {
		rl := r.createLimiter(7, WithoutSlack, WithPer(time.Minute))
